...
reduced_data_dir = '/path/folder/archivereduced'
...
#         images = [f for f in os.listdir(emotion_folder_path) if f.endswith('.png')]
...
if not os.path.exists(reduced_data_dir):
    print(f"The main directory {reduced_data_dir} does not exist.")
    exit()

# Create train and validation folders
train_dir = os.path.join(reduced_data_dir, 'train')
val_dir = os.path.join(reduced_data_dir, 'validation')
...
for emotion in emotions:
    emotion_folder_path = os.path.join(reduced_data_dir, emotion)
    images = [f for f in os.listdir(emotion_folder_path) if f.endswith('.png')]
